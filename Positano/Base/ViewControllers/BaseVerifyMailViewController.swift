...
class BaseVerifyMailViewController: SegueViewController {
    ...
    fileprivate lazy var callMeTimer: Timer = {
        let timer = Timer.scheduledTimer(timeInterval: 1.0, target: self, selector: #selector(BaseVerifyMailViewController.tryCallMe(_:)), userInfo: nil, repeats: true)
        return timer
    }()
